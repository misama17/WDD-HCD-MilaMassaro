...
const speedSlower = document.getElementById("speed-slower");
const speedFaster = document.getElementById("speed-faster");
...
function speechDescription(text) {
    ...
    utteranceSpeech.rate = speechSpeed;   
...
buttonsDialog.forEach(button => {
    button.addEventListener("click", () => {
        ...
        // Brengt focus naar pauze/play knop als er gesproken wordt.
        playpause.focus();
...
function addMessageToWindow(text, sender) {
    ...
    if (sender === "bot") {
        ...
        // na versturen focus naar play/pause knop
        playpause.focus();
    }
}
...
// ChatGPT: maak berekening om de snelheid van speechSpeed aan te passen.
speedSlower.addEventListener("click", () => {
    speechSpeed = Math.max(speechSpeed - 0.25, 0.5);
    speechDescription(currentText);
});

speedFaster.addEventListener("click", () => {
    speechSpeed = Math.min(speechSpeed + 0.25, 2);
    speechDescription(currentText);
});
...
document.addEventListener("keydown", (event) => {
    ...
    if (event.code === "Digit2" || event.code === "Numpad2" && !isTyping) {
    ...
    // replay
    if (event.code === "Digit3" || event.code === "Numpad3" && !isTyping) {
        if(currentText) {
            speechDescription(currentText);
            playpause.textContent = "Pause";
        }
    ...
    // slower
    if (event.code === "Digit1" || event.code === "Numpad1" && !isTyping) {
        event.preventDefault();
    ...
    // faster
    if (event.code === "Digit4" || event.code === "Numpad4" && !isTyping) {
        event.preventDefault();
